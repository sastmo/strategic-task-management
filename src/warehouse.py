...
def ensure_database_objects(conn: psycopg.Connection) -> None:
    statements = [
    ...
    if not staged_frame.empty:
        staged_rows = [
            (
                ...
                bool(row["paused"]),
                str(row["recordHash"]),
                str(row["_source_name"]),
                str(row["_source_kind"]),
...
def calculate_snapshot_counts(conn: psycopg.Connection, *, run_id: int) -> dict[str, int]:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """
            SELECT
...
def write_change_history(conn: psycopg.Connection, *, run_id: int) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
def upsert_current_snapshot(conn: psycopg.Connection, *, run_id: int) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
def merge_staged_data(
    ...
    *,
    ...
    counts = calculate_snapshot_counts(conn, run_id=run_id)
    write_change_history(conn, run_id=run_id)
    upsert_current_snapshot(conn, run_id=run_id)
    deleted_count = mark_deleted_records(
        conn,
        run_id=run_id,
...
def prune_old_staging_data(conn: psycopg.Connection, *, keep_days: int = 30) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
